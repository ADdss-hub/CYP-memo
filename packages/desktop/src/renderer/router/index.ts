/**
 * CYP-memo 桌面客户端路由配置
 * Desktop client router configuration
 * 
...
const routes: RouteRecordRaw[] = [
  ...
  },
  {
    path: '/register',
    name: 'register',
    component: () => import('@app-views/auth/RegisterView.vue'),
    meta: {
      requiresGuest: true,
      title: '注册',
    },
  ...
  },
  {
    ...
    component: () => import('@app-views/auth/RecoverAccountView.vue'),
    meta: {
      requiresGuest: true,
      title: '找回账号',
    },
  },
  {
    ...
    component: () => import('@app-views/auth/ResetPasswordNewView.vue'),
    meta: {
      requiresGuest: true,
      title: '重置密码',
    },
  ...
  },
  {
    path: '/memos',
    name: 'memos',
    component: () => import('@app-views/memo/MemoListView.vue'),
    meta: {
      requiresAuth: true,
  ...
  },
  {
    path: '/memos/new',
    name: 'memo-new',
    component: () => import('@app-views/memo/MemoEditView.vue'),
    meta: {
      requiresAuth: true,
      requiredPermissions: [Permission.MEMO_MANAGE],
      title: '新建备忘录',
  ...
  },
  {
    path: '/memos/:id',
    name: 'memo-detail',
    component: () => import('@app-views/memo/MemoDetailView.vue'),
    meta: {
      requiresAuth: true,
      requiredPermissions: [Permission.MEMO_MANAGE],
      title: '备忘录详情',
  ...
  // 统计
  {
    path: '/statistics',
    name: 'statistics',
    component: () => import('@app-views/StatisticsView.vue'),
    meta: {
      requiresAuth: true,
  ...
  // 附件管理
  {
    path: '/attachments',
    name: 'attachments',
    component: () => import('@app-views/AttachmentsView.vue'),
    meta: {
      requiresAuth: true,
  ...
  // 分享管理
  {
    path: '/shares',
    name: 'shares',
    component: () => import('@app-views/share/ShareManageView.vue'),
    meta: {
      requiresAuth: true,
      requiredPermissions: [Permission.MEMO_MANAGE],
      title: '分享管理',
  ...
  },
  {
    path: '/share/:id',
    name: 'share-view',
    component: () => import('@app-views/share/ShareView.vue'),
    meta: {
      title: '查看分享',
    },
  },
  ...
  // 备忘录数据管理
  {
    path: '/memo-data',
    name: 'memo-data',
    component: () => import('@app-views/MemoDataView.vue'),
    meta: {
      requiresAuth: true,
      requiredPermissions: [Permission.MEMO_MANAGE],
      title: '备忘录数据管理',
...
/**
 * 创建路由实例
 * 使用 Hash 模式以兼容 Electron file:// 协议
 */
...
async function checkFirstLaunch(): Promise<boolean> {
  ...
  // 使用全局 Window 接口中声明的 electronAPI
...
router.beforeEach(async (to, from, next) => {
  ...
  if (firstLaunch && !setupCompleted && to.name !== 'setup') {
    // 首次启动，重定向到设置页面
    next({ name: 'setup' })
    return
  }
  
  // 如果已完成设置但访问设置页面，重定向到登录
  if (to.name === 'setup' && (!firstLaunch || setupCompleted)) {
    next({ name: 'login' })
    return
  }

  // 尝试自动登录（仅在首次访问且未认证时）
  if (!authStore.isAuthenticated && !authStore.isLoading && from.name === undefined && to.name !== 'setup') {
